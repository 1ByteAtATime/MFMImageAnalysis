...
A = importdata(['data_TmB4',sprintf('%03d',i),'.mat']);
y = abs(fftshift(fft2(A)));
% y = fft2(A,2^nextpow2(100),2^nextpow2(200));
...
figure('Visible','off');
annotation('textbox',[.2 .5 .3 .3],'String',[num2str(field(i)), ' ','T', ',', ' ', num2str(temp(i)), ' ', 'K'], 'FitBoxToText','on');
imagesc(y);

% fftB=fft2(double(A));
% fftB=fft2(double(A),2^nextpow2(100),2^nextpow2(200));
% F1=log(abs(fftshift(fftB)));
% surf(F1), shading flat;
axis([230 285 230 285]);
% axis([120 140 50 80]);
% axis([230 285 230 285 0 15]);
saveas(gcf, strcat('F:\TmB4\image6\images\planefit22_15Hz_default\FFT\', num2str(i)),'jpg');
